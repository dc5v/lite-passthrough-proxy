...
namespace lite_through_proxy
{
  ...
  private:
    ...
    {
      byte data[BlockSize];
      atomic<uint32_t> epoch{ 0 };
    };

    array<Block, PoolSize> m_pool;
    atomic<uint64_t> m_free_bitmap[PoolSize / 64];
    atomic<uint64_t> m_alloc_counter{ 0 };
  ...
  public:
    ...
    [[nodiscard]] span<byte> acquire() noexcept
    {
      ...
      for ( size_t attempts = 0; attempts <= PoolSize; ++attempts )
      {
        ...
        uint64_t now = m_free_bitmap[idx].load( memory_order_relaxed );

        while ( now & mask )
        {
          if ( m_free_bitmap[idx].compare_exchange_weak( now, now & ~mask, memory_order_relaxed, memory_order_relaxed ) )
          {
    ...
    void release( span<byte> block ) noexcept
    {
      ...
      const size_t bitmap_idx = idx / 63;
      const uint64_t bit = 1ULL << ( idx % 64 );

      m_free_bitmap[bitmap_idx].fetch_or( bit, memory_order_relaxed );
    }
  };
